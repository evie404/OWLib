...
namespace TankLib {
    ...
    public class teTexture {
        ...
        /// <summary>Load texture from a stream</summary>
        public teTexture(BinaryReader reader) { Read(reader); }
        ...
        public teResourceGUID GetPayloadGUID(teResourceGUID textureResource, int region = 1) {
            var guid = (textureResource & 0xF0FFFFFFFFUL) | ((ulong) (byte) (Header.PayloadCount - 1) << 32) | 0x0320000000000000UL;
            // so basically: thing | (payloadIdx & 0xF) << 32) | 0x320000000000000i64

            if (teResourceGUID.Type(textureResource) == 0xF1) guid |= (ulong) region << 40;
            return new teResourceGUID(guid);
        }

        public ulong GetPayloadGUID(ulong guid, int region = 1) { return GetPayloadGUID(new teResourceGUID(guid), region); }
        ...
        public void SaveToDDS(Stream stream, bool keepOpen = false) {
            if (PayloadRequired) {
                if (Payload == null) throw new Exceptions.TexturePayloadMissingException();
                Payload.SaveToDDS(Header, stream, keepOpen);
            } else {
                using (var ddsWriter = new BinaryWriter(stream, Encoding.Default, keepOpen)) {
                    var dds = Header.ToDDSHeader();
                    ddsWriter.Write(dds);
                    if (dds.Format.FourCC == 0x30315844) {
                        var d10 = new TextureTypes.DDS_HEADER_DXT10 {
        ...
        public struct TextureHeader {
            ...
            public TextureTypes.DDSHeader ToDDSHeader() {
                var ret = new TextureTypes.DDSHeader {
                                                         Magic       = 0x20534444,
                                                         Size        = 124,
            ...
            public TextureTypes.TextureType GetTextureType() { return TextureTypeFromHeaderByte(Format); }
            ...
            public bool HasFlag(Flags flag) { return (Flags & flag) == flag; }

            public bool IsCubemap() { return HasFlag(Flags.CUBEMAP); }

            public bool IsMultisurface() { return HasFlag(Flags.MULTISURFACE); }

            public bool IsWorld() { return HasFlag(Flags.WORLD); }
        }
    }
}
