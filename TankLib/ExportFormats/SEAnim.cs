...
namespace TankLib.ExportFormats {
    ...
    public class SEAnim : IExportFormat {
        ...
        public SEAnim(teAnimation animation) { Animation = animation; }
        ...
        public void Write(Stream stream) {
            ...
            foreach (var boneAnimation in Animation.BoneAnimations) {
                if (boneAnimation.Positions.Count > 0) everHas |= SEAnimPresence.BoneLocation;
                if (boneAnimation.Scales.Count    > 0) everHas |= SEAnimPresence.BoneScale;
                if (boneAnimation.Rotations.Count > 0) everHas |= SEAnimPresence.BoneRotation;
            }

            var frameCount = Animation.InfoTableSize - 1;
            ...
            using (var writer = new BinaryWriter(stream)) {
                ...
                foreach (var boneID in Animation.BoneList) WriteString(writer, OverwatchModel.GetBoneName((uint) boneID));

                foreach (var boneAnimation in Animation.BoneAnimations) {
                    writer.Write((byte) 0);

                    if (everHas.HasFlag(SEAnimPresence.BoneLocation)) WriteFrames3D(writer, frameWidth, boneAnimation.Positions, 2.54f);

                    if (everHas.HasFlag(SEAnimPresence.BoneRotation)) WriteFrames4D(writer, frameWidth, boneAnimation.Rotations);
        ...
        private static void WriteFrames3D(BinaryWriter            writer,
                                          byte                    frameWidth,
                                          Dictionary<int, teVec3> frames,
                                          float                   fac = 1.0f) {
