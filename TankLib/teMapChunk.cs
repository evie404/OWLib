...
namespace TankLib {
    ...
    public abstract class teMapChunk {
        ...
        [SuppressMessage("ReSharper", "VirtualMemberCallInConstructor")]
        public teMapChunk(BinaryReader reader) { Read(reader); }
    ...
    public class teMapPlaceableData : teMapChunk {
        ...
        protected override void Read(BinaryReader reader) {
            ...
            if (Header.PlaceableOffset > 0) {
                ...
                for (var i = 0; i < Header.PlaceableCount; i++) {
                    var beforePos = reader.BaseStream.Position;

                    var commonStructure = reader.Read<CommonStructure>();
                ...
                if (CommonStructures.Length > 0 &&
                    CommonStructures[0]
                        .Type ==
                    teMAP_PLACEABLE_TYPE.ENTITY &&
                    Header.InstanceDataOffset > 0) {
                    var execCount = 0;
                    ...
                    foreach (var placeable in Placeables) {
                        ...
                        for (var i = 0; i < entity.Header.InstanceDataCount; i++) {
                            var beforePos = reader.BaseStream.Position;
                            try {
                                var structuredData = new teStructuredData(reader);
                    ...
                    if (execCount > 0) Debugger.Log(0, "teMapChunk", $"Threw {execCount} exceptions when trying to parse entity instance data\r\n");
                }
            }
        }

        private void AlignPosition(long start, BinaryReader reader, teStructuredData stu) {
            var maxOffset = stu.InstanceInfoV1.Max(x => x.Offset) + start;
            AlignPositionInternal(reader, maxOffset               + 8);
        }

        private void AlignPositionInternal(BinaryReader reader, long pos) {
            for (var i = pos; i < reader.BaseStream.Length; i++) {
    ...
    public class teMapPlaceableEntity : IMapPlaceable {
        ...
        public void Read(BinaryReader reader) { Header = reader.Read<Structure>(); }
    ...
    public class teMapPlaceableSingleModel : IMapPlaceable {
        ...
        public void Read(BinaryReader reader) { Header = reader.Read<teMapPlaceableModel.Structure>(); }
    }
    ...
    public class teMapPlaceableDummy : IMapPlaceable {
        ...
        public unsafe teMapPlaceableDummy(int size) { Size = size - sizeof(teMapPlaceableData.CommonStructure); }

        public teMAP_PLACEABLE_TYPE Type => teMAP_PLACEABLE_TYPE.UNKNOWN;

        public void Read(BinaryReader reader) { Data = reader.ReadBytes(Size); }
    }
    ...
    public class teMapPlaceableManager {
        ...
        public void AddAssemblyTypes(Assembly assembly) {
            foreach (var type in GetAssemblyTypes<IMapPlaceable>(assembly)) {
        ...
        private void AddType(Type type) {
            var instance = (IMapPlaceable) Activator.CreateInstance(type);
        ...
        public IMapPlaceable CreateType(teMapPlaceableData.CommonStructure commonStructure, BinaryReader reader) {
            ...
            if (Types.TryGetValue(commonStructure.Type, out var placeableType))
                value = (IMapPlaceable) Activator.CreateInstance(placeableType);
            else if (_misingTypes.Add(commonStructure.Type)) Debugger.Log(0, "teMapPlaceableManager", $"Unhandled placeable type: {commonStructure.Type}\r\n");
            value.Read(reader);
            return value;
        }
    }
