...
namespace TankLib.STU {
    ...
    public class STUInstance : ISerializable_STU {
        ...
        public void Deserialize(teStructuredData assetFile, STUField_Info field = null) {
            ...
            var instanceHash = teStructuredData.Manager.InstancesInverted[GetType()];
            var stuAttribute = teStructuredData.Manager.InstanceAttributes[instanceHash];
            var fields       = teStructuredData.Manager.FieldAttributes[instanceHash];

            if (assetFile.Format == teStructuredDataFormat.V2) {
                var fieldBagIdx = data.ReadInt32();
                ...
                foreach (var stuField in assetFile.FieldInfoBags[fieldBagIdx]) {
                    var fieldSize                 = stuField.Size;
                    if (fieldSize == 0) fieldSize = data.ReadInt32();
                    var startPosition             = data.BaseStream.Position;
                ...
                //    //Debugger.Break();
                //}
                var fieldOrder = teStructuredData.Manager.InstanceFields[instanceHash];

                foreach (var fieldHash in fieldOrder) {
                    //long fieldStart = data.BaseStream.Position;

                    var stuField = new STUField_Info { Hash = fieldHash, Size = -1 };
        ...
        protected void DeserializeField(teStructuredData                                             assetFile,
                                        STUField_Info                                                fieldInfo,
                                        Dictionary<uint, KeyValuePair<FieldInfo, STUFieldAttribute>> fields,
                                        STUAttribute                                                 stuAttribute) {
            if (!fields.TryGetValue(fieldInfo.Hash, out var field)) {
                var name = stuAttribute.Name ?? $"STU_{stuAttribute.Hash:X8}"; // todo: dis slow
                Debugger.Log(0, "STUInstance", $"Unhandled field: {name}:{fieldInfo.Hash:X8} (size: {fieldInfo.Size})\r\n");
                return;
            }

            if (assetFile.Format == teStructuredDataFormat.V1) assetFile.DynData = assetFile.Data;

            var reader = teStructuredData.Manager.FieldReaders[field.Value.ReaderType];

            if (field.Key.FieldType.IsArray) {
                var elementType = field.Key.FieldType.GetElementType();
                ...
                if (assetFile.Format == teStructuredDataFormat.V2) {
                    if (fieldInfo.Size == 0 || reader is InlineInstanceFieldReader) { // inline
                        ...
                        for (var i = 0; i != size; ++i) reader.Deserialize_Array(teStructuredData.Manager, assetFile, fieldInfo, array, i);
                    } else {
                        var offset = data.ReadInt32();
                        if (offset == -1) return;
                        dynData.Seek(offset);
                        var size = dynData.ReadInt32();
                        if (size == 0) return;
                        array = Array.CreateInstance(elementType, size);
                        var unknown = dynData.ReadUInt32();
                    ...
                    if (offset <= 0) {
                        ...
                        var count      = data.ReadInt64();
                        var dataOffset = data.ReadInt64();
                        ...
                        if (count != -1 && dataOffset > 0) {
                            ...
                            for (var i = 0; i != count; ++i) reader.Deserialize_Array(teStructuredData.Manager, assetFile, fieldInfo, array, i);
                        } else {
                            array = null;
                        }
                    }
