// Instance generated by TankLibHelper.InstanceBuilder

using TankLib.STU.Types.Enums;

// ReSharper disable All
namespace TankLib.STU.Types {
    [STU(0xD797394C, "STUGameModeTeam")]
    public class STUGameModeTeam : STUInstance {
        [STUField(0x170AA4B8)]
        public int m_170AA4B8;

        [STUField(0x6F71E9AA, ReaderType = typeof(InlineInstanceFieldReader))]
        public STUGameModeVarValuePair[] m_6F71E9AA;

        [STUField(0x7FA93ED4)]
        public int m_7FA93ED4;

        [STUField(0x8B3CD15B)]
        public int m_8B3CD15B;

        [STUField(0x59C86C8D, "m_availableHeroes", ReaderType = typeof(EmbeddedInstanceFieldReader))]
        public STU_5427ADC1 m_availableHeroes;

        [STUField(0x76E8C82A, "m_bodyScript")]
        public teStructuredDataAssetRef<STU_6BE90C5C> m_bodyScript;

        [STUField(0xEA2B516F, "m_bodyVars", ReaderType = typeof(InlineInstanceFieldReader))]
        public STUGameModeBodyVars[] m_bodyVars;

        [STUField(0xA2781AA4, "m_controllerScript")]
        public teStructuredDataAssetRef<STU_6BE90C5C> m_controllerScript;

        [STUField(0x33B0B2B6, "m_team")]
        public TeamIndex m_team;
    }
}
