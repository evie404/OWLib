...
namespace TankLib.Chunks {
    ...
    public class teModelChunk_Cloth : IChunk {
            ...
            using (var reader = new BinaryReader(input)) {
                ...
                if (Header.Offset > 0) {
                    ...
                    for (ulong i = 0; i < Header.Count; i++) {
                        ...
                        var nextStartPos = reader.BaseStream.Position;

                        if (Descriptors[i]
                                .Section8Offset !=
                            4574069944263674675) { // todo: wtf
                            reader.BaseStream.Position = Descriptors[i]
                                .Section8Offset;
                            ...
                            for (var nodeIndex = 0;
                                 ...
                                 Descriptors[i]
                                     .DriverNodeCount;
                                 nodeIndex++) NodeBones[i][nodeIndex] = reader.ReadInt16();
                        }
                        ...
                        if (Descriptors[i]
                            ...
                            Descriptors[i]
                                .Section1Offset !=
                            4692750811720056850) { // todo: wtf2
                            Nodes[i] = new ClothNode[Descriptors[i]
                                .DriverNodeCount];
                            reader.BaseStream.Position = Descriptors[i]
                                .Section1Offset;
                            for (var nodeIndex = 0;
                                 ...
                                 Descriptors[i]
                                     .DriverNodeCount;
                                 nodeIndex++) Nodes[i][nodeIndex] = new ClothNode(reader);
                        }
        ...
        public short[] CreateFakeHierarchy(teModelChunk_Skeleton skeleton, out Dictionary<int, ClothNode> nodeMap) {
            var hierarchy = (short[]) skeleton.Hierarchy.Clone();
            nodeMap = new Dictionary<int, ClothNode>();

            uint clothIndex = 0;
            foreach (var nodeCollection in Nodes) {
                if (nodeCollection == null) continue;
                var nodeIndex = 0;
                foreach (var node in nodeCollection) {
                    int parentRaw = node.VerticalParent;
                    if (NodeBones[clothIndex]
                            .ContainsKey(nodeIndex) &&
                        NodeBones[clothIndex]
                            .ContainsKey(parentRaw)) {
                        if (NodeBones[clothIndex][nodeIndex] != -1) {
                            hierarchy[NodeBones[clothIndex][nodeIndex]] = NodeBones[clothIndex][parentRaw];
                            if (NodeBones[clothIndex][parentRaw] == -1) {
                                var weightedBone = node.Bones.Aggregate((i1, i2) => i1.Weight > i2.Weight ? i1 : i2);
        ...
        public unsafe struct ClothDesc {
            ...
            public string StringName {
                get {
                    fixed (byte* n = Name) {
                        return Encoding.ASCII.GetString(n, 32)
                                       .Split('\0')[0];
                    }
                }
            }
        ...
        public class ClothNode {
            ...
            public ClothNode(BinaryReader reader) {
                ...
                Y = reader.ReadSingle();
                Z = reader.ReadSingle();
                var zero = reader.ReadUInt32(); // zero
                var x2   = reader.ReadSingle();
                var y2   = reader.ReadSingle();
                var z2   = reader.ReadSingle();

                if (zero != 0) throw new InvalidDataException($"HTLC: zero != 0 ({zero})");

                if (System.Math.Abs(X - x2) > 0.01 || System.Math.Abs(Y - y2) > 0.01 || System.Math.Abs(Z - z2) > 0.01) throw new InvalidDataException($"HTLC: location is different: {X}:{x2} {Y}:{y2} {Z}:{z2}");

                reader.BaseStream.Position = nodeStart + 0x15c;
                var ind1 = reader.ReadInt16();
                var ind2 = reader.ReadInt16();
                var ind3 = reader.ReadInt16();
                var ind4 = reader.ReadInt16();
                ...
                var weight4 = reader.ReadSingle();

                Bones = new[] { new ClothNodeWeight(ind1, weight1), new ClothNodeWeight(ind2, weight2), new ClothNodeWeight(ind3, weight3), new ClothNodeWeight(ind4, weight4) };
                ...
                if (IsChild && ChainNumber == -1) throw new InvalidDataException("HTLC: node is child but not in a chain");
