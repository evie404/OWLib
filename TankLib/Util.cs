...
namespace TankLib {
    public class Util {
        ...
        public static string GetVersion() {
            var asm = Assembly.GetAssembly(typeof(teResourceGUID));
        ...
        public static int GetMagicBytes(params byte[] chars) {
            ...
            if (chars.Length == 3)
                ...
                v = Enumerable.Repeat((byte) 0, chars.Length - 4)
                              .Concat(v);
        ...
        public static int GetMagicBytesBE(params byte[] chars) {
            ...
            var v = chars.AsEnumerable();

            if (chars.Length == 3)
                v                        = v.Append((byte) 0);
            else if (chars.Length < 4) v = v.Concat(Enumerable.Repeat((byte) 0, chars.Length - 4));
        ...
        public static int GetMagicBytes(params char[] chars) {
            return GetMagicBytes(chars.Select(x => (byte) x)
                                      .ToArray());
        }

        public static int GetMagicBytesBE(params char[] chars) {
            return GetMagicBytesBE(chars.Select(x => (byte) x)
                                        .ToArray());
        }
        ...
        public static string GetVersion(Assembly asm) {
            ...
            var file   = asm.GetCustomAttribute<AssemblyFileVersionAttribute>();
            if (attrib == null) return file.Version;
