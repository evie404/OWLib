...
namespace TankLib {
    ...
    public class teTexturePayload {
        ...
        public teTexturePayload(teTexture parent, Stream payloadStream) {
            using (var dataReader = new BinaryReader(payloadStream)) {
                ...
                if (parent.Header.Format > 72) {
                    ...
                    for (var i = 0; i < Size; ++i) { // todo: can make this faster
        ...
        public void SaveToDDS(teTexture.TextureHeader parentHeader, Stream stream, bool keepOpen = false) {
            using (var ddsWriter = new BinaryWriter(stream, Encoding.Default, keepOpen)) {
                var dds = parentHeader.ToDDSHeader();
                ddsWriter.Write(dds);
