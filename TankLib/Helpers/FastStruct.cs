...
namespace TankLib.Helpers {
    public static class FastStruct<T> where T : struct {
        ...
        private static LoadFromByteRefDelegate BuildLoadFromByteRefMethod() {
            var methodLoadFromByteRef = new DynamicMethod("LoadFromByteRef<" + typeof(T).FullName + ">", typeof(T), new[] { typeof(byte).MakeByRefType() }, typeof(FastStruct<T>));
        ...
        private static CopyMemoryDelegate BuildCopyMemoryMethod() {
            var methodCopyMemory = new DynamicMethod("CopyMemory<" + typeof(T).FullName + ">", typeof(void), new[] { typeof(T).MakeByRefType(), typeof(byte).MakeByRefType(), typeof(int) }, typeof(FastStruct<T>));
        ...
        private static RevCopyMemoryDelegate BuildRevCopyMemoryMethod() {
            var methodCopyMemory = new DynamicMethod("RevCopyMemory<" + typeof(T).FullName + ">", typeof(void), new[] { typeof(byte).MakeByRefType(), typeof(T).MakeByRefType(), typeof(int) }, typeof(FastStruct<T>));
        ...
        public static T ArrayToStructure(byte[] src) { return LoadFromByteRef(ref src[0]); }
