...
namespace TankLib {
    ...
    public struct teChunkDataHeader {
        ...
        public string StringIdentifier =>
            Encoding.ASCII.GetString(BitConverter.GetBytes(ID))
                    .ReverseXor();

        public new string ToString() { return base.ToString() + $" ({StringIdentifier})"; }
    }
    ...
    public class teChunkedData {
        ...
        public teChunkedData(Stream input, bool keepOpen = false) {
            ...
            using (var reader = new BinaryReader(input, Encoding.UTF8, keepOpen)) {
                Read(reader);
            }
        }
        ...
        public teChunkedData(BinaryReader reader) { Read(reader); }
        ...
        private unsafe void Read(BinaryReader reader) {
            ...
            var next = reader.BaseStream.Position - start; // rel stream pos

            while (next < Header.Size) {
                var entry = reader.Read<teChunkDataEntry>();
                next += entry.Size + sizeof(teChunkDataEntry);

                var chunk = Manager.CreateChunkInstance(entry.StringIdentifier, Header.StringIdentifier);
                if (chunk != null) {
                    var dataStream = new MemoryStream(entry.Size);
        ...
        public bool HasChunk<T>() where T : IChunk { return Chunks.Any(x => x is T); }

        public T GetChunk<T>() where T : IChunk { return (T) Chunks.FirstOrDefault(x => x is T); }

        public IEnumerable<T> GetChunks<T>() where T : IChunk { return Chunks.OfType<T>(); }

        // Behold, the most disgusting LINQ you've ever seen.
        public IChunk GetChunkByTag(string tag) {
            return Chunks.ElementAtOrDefault(ChunkTags.Select((x, y) => new { Value = x, Index = y })
                                                      .FirstOrDefault(x => x.Value == tag)
                                                      ?.Index ??
                                             -1);
        }

        public IEnumerable<IChunk> GetChunksByTag(string tag) {
            var indices = ChunkTags.Select((x, y) => new { Value = x, Index = y })
                                   .Where(x => x.Value == tag)
                                   .Select(x => x.Index);
            return Chunks.Select((x, y) => new { Value = x, Index = y })
    ...
    public class teChunkManager {
        ...
        public void AddAssemblyChunks(Assembly assembly) {
            ...
            var types = assembly.GetTypes()
                                .Where(type => type != t && t.IsAssignableFrom(type))
                                .ToList();
        ...
        public IChunk CreateChunkInstance(string id, string rootID) {
            if (ChunkTypes.TryGetValue(id, out var chunkType)) return (IChunk) Activator.CreateInstance(chunkType);
