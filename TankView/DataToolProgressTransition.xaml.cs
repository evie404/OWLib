...
namespace TankView {
    public partial class DataToolProgressTransition : Window {
        ...
        public DataToolProgressTransition(IAwareTool tool) : this(tool.GetToolControl,
                                                                  tool.GetType()
                                                                      .GetCustomAttributes<ToolAttribute>()
                                                                      .FirstOrDefault()
                                                                      ?.Name ??
                                                                  "DataTool") { }
        ...
        public DataToolProgressTransition(Func<ProgressWorker, SynchronizationContext, Window, Task<Control>> fn, string name) {
            ...
            var window = new DataToolSimView { ModuleName = name, Visibility = Visibility.Hidden };

            var t = new Thread(() => {
                                   Control control = null;
                                   try {
                                       control = fn(_progressWorker, window.ViewContext, window)
                                                 .GetAwaiter()
                                                 .GetResult();
                                   } catch (Exception e) {
