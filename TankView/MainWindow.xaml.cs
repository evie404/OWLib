...
namespace TankView {
    ...
    public partial class MainWindow : INotifyPropertyChanged {
        ...
        public bool IsReady {
            ...
            set {
                ready = value;
                if (value) _progressWorker.ReportProgress(0, "Idle");
        ...
        public bool IsDataToolSafe => IsDataReady && Program.TankHandler?.MainContentManifest?.HashList != null; // todo
        ...
        private void UpdateProgress(object sender, ProgressChangedEventArgs @event) {
            ViewContext.Send(x => {
                                 ...
                                 if (evt.UserState != null && evt.UserState is string state) ProgressInfo.State = state;
        ...
        private void NGDPHostChange(object sender, RoutedEventArgs e) {
            ...
            foreach (var node in NGDPPatchHosts.Where(x => x.Active && x.GetHashCode() != ((PatchHost) menuItem.DataContext).GetHashCode())) node.Active = false;

            CollectionViewSource.GetDefaultView(NGDPPatchHosts)
                                .Refresh();
        }

        private void OpenNGDP(object sender, RoutedEventArgs e) { throw new NotImplementedException(nameof(OpenNGDP)); }
        ...
        private void OpenCASC(object sender, RoutedEventArgs e) {
            ...
            if (dialog.ShowDialog() == CommonFileDialogResult.Ok) OpenCASC(dialog.FileName);
        }
        ...
        private void OpenRecent(object sender, RoutedEventArgs e) {
            ...
            CollectionViewSource.GetDefaultView(RecentLocations)
                                .Refresh();
        ...
        private void OpenAgent(object sender, RoutedEventArgs e) {
            if (sender is MenuItem menuItem) OpenCASC(menuItem.Tag as string);
        }
...
#pragma warning disable 162
            ...
            Task.Run(delegate {
                         try {
                             Program.Client = new ClientHandler(null, ClientArgs);

                             ClientArgs.Online = true;

                             Program.TankHandler = Program.Client.ProductHandler as ProductHandler_Tank;
                         } catch (Exception e) {
...
#pragma warning restore 162
        ...
        private void OpenCASC(string path) {
            ...
            Task.Run(delegate {
                         try {
                             Program.Client = new ClientHandler(path, ClientArgs);
                             ...
                             GCSettings.LatencyMode = GCLatencyMode.Interactive;
                             GC.Collect();
                             if (Settings.Default.LoadManifest) Program.InitTrackedFiles();
                             ViewContext.Send(delegate {
                                                  IsReady = true;
                                                  NotifyPropertyChanged(nameof(IsReady));
                                              },
                                              null);
                         ...
                         if (Program.Client?.AgentProduct?.Uid != null && Program.Client.AgentProduct.Uid != "prometheus") MessageBox.Show($"The branch \"{Program.Client.AgentProduct.Uid}\" is not supported!\nThis might result in failure to load.\nProceed with caution.", "Unsupported Branch", MessageBoxButton.OK, MessageBoxImage.Warning, MessageBoxResult.OK);
                     });
        }
        ...
        private void BuildTree() {
            GUIDTree?.Dispose();
            GUIDTree = new GUIDCollection(Program.Client, Program.TankHandler, _progressWorker);
            NotifyPropertyChanged(nameof(GUIDTree));
        }
        ...
        private void OpenOrFocusSimView(object sender, RoutedEventArgs e) {
            var view = Application.Current.Windows.OfType<DataToolSimView>()
                                  .FirstOrDefault();
            if (view != null) {
                view.Show();
                view.Focus();
            ...
            var instance = Application.Current.Windows.OfType<DataToolListView>()
                                      .FirstOrDefault() ??
                           new DataToolListView();
            instance.Show();
            instance.Focus();
        }

        private void ExtractFiles(object sender, RoutedEventArgs e) {
            var files = FolderItemList.SelectedItems.OfType<GUIDEntry>()
                                      .ToArray();
            if (files.Length == 0)
                files = FolderItemList.Items.OfType<GUIDEntry>()
                                      .ToArray();

            if (files.Length == 0) return;

            var dialog = new CommonOpenFileDialog { IsFolderPicker = true, EnsurePathExists = true };
            if (dialog.ShowDialog() == CommonFileDialogResult.Ok) ExtractFiles(dialog.FileName, files);
        }
        ...
        private void ExtractFiles(string outPath, IEnumerable<GUIDEntry> files) {
            ...
            var                    directories = new HashSet<string>(guidEntries.Select(x => Path.Combine(outPath, Path.GetDirectoryName(x.FullPath.Substring(1)) ?? string.Empty)));
            ...
            Task.Run(delegate {
                         var c = 0;
                         var t = guidEntries.Count();
                         _progressWorker?.ReportProgress(0, "Saving files...");
                         Parallel.ForEach(guidEntries,
                                          new ParallelOptions { MaxDegreeOfParallelism = 4 },
                                          entry => {
                                              c++;
                                              if (c % ((int) (t * 0.005) + 1) == 0) _progressWorker?.ReportProgress((int) (c / (float) t * 100));

                                              try {
                                                  using (var i = IOHelper.OpenFile(entry))
        ...
        private void ExtractFolder(Folder folder, ref List<GUIDEntry> files) {
            ...
            foreach (var f in folder.Folders) ExtractFolder(f, ref files);
        }
        ...
        private void ExtractFolder(object sender, RoutedEventArgs e) {
            var dialog = new CommonOpenFileDialog { IsFolderPicker = true, EnsurePathExists = true };
            if (dialog.ShowDialog() == CommonFileDialogResult.Ok) ExtractFolder(dialog.FileName, (sender as FrameworkElement)?.DataContext as Folder);
        }
