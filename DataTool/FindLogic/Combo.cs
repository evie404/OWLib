...
namespace DataTool.FindLogic {
    public static class Combo {
        ...
        public static bool RemoveDuplicateVoiceSetEntries(ComboInfo     @base,
                                                          ref ComboInfo target,
                                                          ulong         voiceSet,
                                                          ulong         targetVoiceSet) {
            if (!@base.VoiceSets.ContainsKey(voiceSet) || !target.VoiceSets.ContainsKey(targetVoiceSet)) return false;

            var keys = new HashSet<ulong>();
            foreach (var pair in @base.VoiceSets[voiceSet]
                                      .VoiceLineInstances)
            foreach (var voice in pair.Value)
            foreach (var guid in voice.SoundFiles)
                keys.Add(guid);

            var hasData = false;

            // we have to call toarray here to "freeze" the GC stack and allow us to modify the "original" without C# bitching.
            foreach (var pair in target.VoiceSets[targetVoiceSet]
                                       .VoiceLineInstances.ToArray()) {
                var newSet = new HashSet<VoiceLineInstanceInfo>();
                foreach (var voice in pair.Value) {
                    foreach (var guid in voice.SoundFiles.ToArray()) // and here
                        if (!keys.Add(guid))
                ...
                target.VoiceSets[targetVoiceSet]
                      .VoiceLineInstances[pair.Key] = newSet;
            }

            return hasData;
        }

        public static ComboInfo Find(ComboInfo                info,
                                     ulong                    guid,
                                     Dictionary<ulong, ulong> replacements = null,
                                     ComboContext             context      = null) {
            ...
            if (info.Config.FullLog) Console.Out.WriteLine($"[DataTool.FindLogic.Combo]: Searching in {GetFileName(guid)}");
            ...
            switch (guidType) {
                case 0x2:
                    ...
                    var mapInfo = new MapInfoNew(guid);
                ...
                case 0x3:
                    if (info.Config == null || info.Config.DoExistingEntities == false)
                        if (info.Entities.ContainsKey(guid))
                            break;

                    var entityDefinition = GetInstance<STUEntityDefinition>(guid);
                    ...
                    if (context.ChildEntityIdentifier != 0) {
                        if (!info.EntitiesByIdentifier.ContainsKey(context.ChildEntityIdentifier)) info.EntitiesByIdentifier[context.ChildEntityIdentifier] = new HashSet<ulong>();

                        info.EntitiesByIdentifier[context.ChildEntityIdentifier]
                            .Add(guid);
                    }

                    if (entityDefinition.m_childEntityData != null) {
                        entityInfo.Children = new List<ChildEntityReferenceNew>();
                        foreach (var childEntityDefinition in entityDefinition.m_childEntityData) {
                            if (childEntityDefinition == null) continue;
                            var childContext = new ComboContext { ChildEntityIdentifier = childEntityDefinition.m_49F782CE };
                            Find(info, (ulong) childEntityDefinition.m_child, replacements, childContext);
                            if (info.Entities.ContainsKey(GetReplacement((ulong) childEntityDefinition.m_child, replacements))) entityInfo.Children.Add(new ChildEntityReferenceNew(childEntityDefinition, replacements));
                        }
                    }

                    if (entityDefinition.m_componentMap != null) {
                        var components = entityDefinition.m_componentMap.Values.OrderBy(x => x?.GetType() != typeof(STUModelComponent) && x?.GetType() != typeof(STUEffectComponent))
                                                         .ToArray();
                        ...
                        foreach (var component in components) {
                            ...
                            if (component is STUModelComponent modelComponent) {
                                ...
                                if (statescriptComponent.m_B634821A != null)
                                    foreach (var graphWithOverrides in statescriptComponent.m_B634821A)
                                ...
                                if (weaponComponent.m_weapons != null)
                                    foreach (var weaponDefinition in weaponComponent.m_weapons) {
                                ...
                                foreach (var unkComponent2Entry in unkComponent2.m_entries) Find(info, unkComponent2Entry.m_animation, replacements, entityContext);
                            }
                        }
                    }
                    ...
                    if (entityInfo.VoiceSet != 0)
                        foreach (var entityAnimation in entityInfo.Animations) {
                            var entityAnimationInfo = info.Animations[entityAnimation];
                ...
                case 0xF1:
                    ...
                    var textureInfo = new TextureInfoNew(guid);
                    info.Textures[guid] = textureInfo;

                    if (context.Material == 0) textureInfo.Loose = true;
                ...
                case 0x6:
                    ...
                    if (info.Animations.ContainsKey(guid)) {
                        if (context.Model != 0)
                            info.Models[context.Model]
                                .Animations.Add(guid);

                        if (context.Entity != 0)
                            info.Entities[context.Entity]
                                .Animations.Add(guid);

                        break;
                    }

                    var animationInfo = new AnimationInfoNew(guid);

                    var animationContext = context.Clone();
                    animationContext.Animation = guid;

                    using (var animationStream = OpenFile(guid)) {
                        if (animationStream == null) break;
                        using (var animationReader = new BinaryReader(animationStream)) {
                            animationStream.Position = 0;
                            var priority = animationReader.ReadUInt32();
                            animationStream.Position = 8;
                            var fps = animationReader.ReadSingle();
                            animationStream.Position = 0x18L;
                            var effectKey = animationReader.ReadUInt64();
                ...
                case 0x8:
                    if (info.Materials.ContainsKey(guid) &&
                        (info.Materials[guid]
                             .MaterialIDs.Contains(context.MaterialID) ||
                         context.MaterialID == 0)) break;
                    // ^ break if material exists and has id, or id is 0
                    var material = new teMaterial(OpenFile(guid));

                    MaterialInfo materialInfo;
                    if (!info.Materials.ContainsKey(guid)) {
                        materialInfo         = new MaterialInfo(guid) { MaterialData = GetReplacement(material.Header.MaterialData, replacements), MaterialIDs = new HashSet<ulong>() };
                    ...
                    if (context.ModelLook == 0 && context.Model != 0)
                        info.Models[context.Model]
                            .LooseMaterials.Add(guid);

                    var materialContext = context.Clone();
                ...
                case 0xC:
                    ...
                    var modelInfo = new ModelInfoNew(guid);
                ...
                case 0x8E:
                    ...
                    var effectInfo = new EffectParser.EffectInfo { GUID = guid };
                    ...
                    if (guidType == 0xD || guidType == 0x8E) {
                        ...
                        if (context.Entity != 0)
                            info.Entities[context.Entity]
                                .AnimationEffects.Add(guid);
                    }

                    using (var effectStream = OpenFile(guid)) {
                        var chunkedData = new teChunkedData(effectStream);
                        var parser      = new EffectParser(chunkedData, guid);

                        ulong lastParticleModel = 0;

                        foreach (var chunk in parser.GetChunks()) {
                            parser.Process(effectInfo, chunk, replacements);

                            if (chunk.Value is teEffectComponentModel model) {
                                var dmceContext = new ComboContext { Model = GetReplacement(model.Header.Model, replacements) };
                                ...
                                var neceContext = new ComboContext { ChildEntityIdentifier = entity.Header.Identifier };
                                ...
                                foreach (var ceceEntity in info.EntitiesByIdentifier[entityControl.Header.Identifier]) {
                                    var ceceEntityInfo = info.Entities[ceceEntity];
                                    ceceEntityInfo.Animations.Add(GetReplacement(entityControl.Header.Animation, replacements));
                                    if (ceceEntityInfo.Model != 0)
                                        info.Models[ceceEntityInfo.Model]
                                            .Animations.Add(GetReplacement(entityControl.Header.Animation, replacements));
                                }
                                ...
                                var ssceContext = new ComboContext { Model = lastParticleModel };
                ...
                case 0x1A:
                    ...
                    var modelLookInfo = new ModelLookInfo(guid);
                    info.ModelLooks[guid] = modelLookInfo;

                    var modelLookContext = context.Clone();
                    modelLookContext.ModelLook = guid;

                    if (modelLook.m_materials != null) {
                        modelLookInfo.Materials = new HashSet<ulong>();
                        foreach (var modelLookMaterial in modelLook.m_materials) FindModelMaterial(info, modelLookMaterial, modelLookInfo, modelLookContext, replacements);
                    }
                ...
                case 0x1B:
                    ...
                    var configVars = GetInstances<STUConfigVar>(guid);
                    if (configVars == null) break;

                    foreach (var configVar in configVars) Find(info, configVar, replacements, context);
                    ...
                    break;
                case 0x20:
                    var blendTree = GetInstance<STUAnimBlendTree>(guid);
                    foreach (var animNode in blendTree.m_animNodes)
                        ...
                        else if (animNode is STUAnimNode_AnimationPose2d animNodePose2D) Find(info, animNodePose2D?.m_animation?.m_value,    replacements, context);
                    break;
                case 0x21:
                    var blendTreeSet = GetInstance<STUAnimBlendTreeSet>(guid);
                    if (blendTreeSet == null) break;

                    foreach (var externalRef in blendTreeSet.m_externalRefs) Find(info, externalRef, replacements, context);

                    foreach (var blendTreeItem in blendTreeSet.m_blendTreeItems) {
                        ...
                        if (blendTreeItem.m_gameData is STU_7D00A73D animGameDataUnk1)
                            if (animGameDataUnk1.m_animDatas != null)
                                foreach (var gameDataAnimationData in animGameDataUnk1.m_animDatas)
                                    Find(info, gameDataAnimationData.m_9FCB2C8A, replacements, context);

                        if (blendTreeItem?.m_onFinished?.m_slotAnims != null)
                            foreach (var blendTreeSlotAnimation in blendTreeItem.m_onFinished.m_slotAnims)
                ...
                case 0x2C:
                    ...
                    var sound = GetInstance<STUSound>(guid);
                    if (sound == null) break;

                    var soundInfo = new SoundInfoNew(guid);
                    ...
                    if (sound.m_C32C2195 != null) {
                        if (sound.m_C32C2195.m_soundWEMFiles != null) {
                            ...
                            var i = 0;
                            foreach (var soundWemFile in sound.m_C32C2195.m_soundWEMFiles) {
                        ...
                        if (sound.m_C32C2195.m_soundWEMStreams != null) {
                            ...
                            var i = 0;
                            foreach (var soundWemStream in sound.m_C32C2195.m_soundWEMStreams) {
                        ...
                        if (sound.m_C32C2195.m_09D4067B != null)
                            foreach (var soundUnk1 in sound.m_C32C2195.m_09D4067B)
                                Find(info, soundUnk1, replacements, context);
                        if (sound.m_C32C2195.m_4587972B != null)
                            foreach (var soundUnk2 in sound.m_C32C2195.m_4587972B)
                ...
                case 0x3F:
                    ...
                    var soundFileInfo = new SoundFileInfo(guid);
                ...
                case 0x5F:
                    ...
                    var voiceSetInfo = new VoiceSetInfo(guid);
                    ...
                    for (var i = 0; i < voiceSet.m_voiceLineInstances.Length; i++) {
                        var voiceLineInstance = voiceSet.m_voiceLineInstances[i];
                        if (voiceLineInstance == null) continue;

                        var voiceLineInstanceInfo = new VoiceLineInstanceInfo {
                        ...
                        if (voiceLineInstance.m_AF226247 != null)
                            foreach (var soundFile in new[] { voiceLineInstance.m_AF226247.m_1485B834, voiceLineInstance.m_AF226247.m_798027DE, voiceLineInstance.m_AF226247.m_A84AA2B5, voiceLineInstance.m_AF226247.m_D872E45C }) {
                        ...
                        if (!voiceSetInfo.VoiceLineInstances.ContainsKey(voiceLineInstanceInfo.VoiceStimulus)) voiceSetInfo.VoiceLineInstances[voiceLineInstanceInfo.VoiceStimulus] = new HashSet<VoiceLineInstanceInfo>();
                        voiceSetInfo.VoiceLineInstances[voiceLineInstanceInfo.VoiceStimulus]
                                    .Add(voiceLineInstanceInfo);
                    }
                ...
                case 0x7C:
                    ...
                    var stringInfo = new StringInfo(guid) { Value = GetString(guid) };

                    info.Strings[guid] = stringInfo;
                    break;
                case 0xA5:
                    // hmm, if existing?
                    var cosmetic = GetInstance<STUUnlock>(guid);
                ...
                case 0xA6:
                    ...
                    var skinOverride = GetInstance<STUSkinTheme>(guid);
                    if (skinOverride?.m_runtimeOverrides == null) break;
                    foreach (var replacement in skinOverride.m_runtimeOverrides) {
                ...
                case 0xA8:
                    // hmm, if existing?
                    var effectLook = GetInstance<STUEffectLook>(guid);
                    foreach (var effectLookMaterialData in effectLook.m_materialData) Find(info, effectLookMaterialData, replacements, context);
                    break;
                case 0xB2:
                    if (info.VoiceSoundFiles.ContainsKey(guid)) break;
                    var voiceSoundFileInfo = new SoundFileInfo(guid);
                    info.VoiceSoundFiles[guid] = voiceSoundFileInfo;
                    break;
                case 0xB3:
                    if (info.MaterialDatas.ContainsKey(guid)) break;
                    var materialDataContext = context.Clone();
                    materialDataContext.MaterialData = guid;

                    var materialDataInfo = new MaterialDataInfo(guid);

                    info.MaterialDatas[guid] = materialDataInfo;

                    var materialData = new teMaterialData(OpenFile(guid));
                    if (materialData.Textures != null) {
                        materialDataInfo.Textures = new Dictionary<ulong, uint>();
                        foreach (var matDataTex in materialData.Textures) {
                    ...
                    break;
                case 0xBF:
                    var lineupPose = GetInstance<STULineupPose>(guid);
                    ...
                    break;
                default:
                    if (UnhandledTypes.Add(guidType)) Debugger.Log(0, "DataTool", $"[DataTool.FindLogic.Combo]: Unhandled type: {guidType:X3}\r\n");
                    break;
            }
        ...
        private static void Find(ComboInfo                        info,
                                 STUStatescriptGraphWithOverrides graphWithOverrides,
            ...
            if (graphWithOverrides.m_1EB5A024 != null)
                foreach (var schemaEntry in graphWithOverrides.m_1EB5A024)
                    Find(info, schemaEntry.m_value, replacements, context);
        }

        private static void Find(ComboInfo                info,
                                 STUConfigVar             configVar,
                                 Dictionary<ulong, ulong> replacements,
                                 ComboContext             context) {
            ...
            if (configVar is STU_8556841E configVarEntity) {
                ...
                if (configVarExpression.m_configVars != null)
                    foreach (var subVar in configVarExpression.m_configVars)
        ...
        private static void FindModelMaterial(ComboInfo                info,
                                              STUModelMaterial         modelMaterial,
                                              ModelLookInfo            modelLookInfo,
                                              ComboContext             modelLookContext,
                                              Dictionary<ulong, ulong> replacements) {
            ...
            var modelMaterialContext = modelLookContext.Clone();
        ...
        public class ComboInfo {
            ...
            public void SetEffectVoiceSet(EffectInfoCombo effect, ulong voiceSet) { effect.Effect.VoiceSet = voiceSet; }
        }
        ...
        public class ComboType {
            ...
            public ComboType(ulong guid) { GUID = guid; }

            public virtual string GetName() { return GetFileName(GUID); }

            public virtual string GetNameIndex() { return $"{GUID & 0xFFFFFFFFFFFF:X12}"; }
        }
        ...
        public class ComboNameable : ComboType {
            ...
            public ComboNameable(ulong guid) : base(guid) {
                ...
                var  index = teResourceGUID.Index(GUID);
                if (!GUIDTable.ContainsKey(type)) return;
                if (GUIDTable[type]
                    .ContainsKey(index)) Name = GUIDTable[type][index];
            }

            public override string GetName() { return GetValidFilename(Name) ?? GetFileName(GUID); }

            public override string GetNameIndex() { return GetValidFilename(Name) ?? $"{GUID & 0xFFFFFFFFFFFF:X12}"; }
        }
        ...
        public class WWiseBankEvent {
            ...
            public Sound.BankObjectEventAction.EventActionType Type;
        }
