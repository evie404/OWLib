...
namespace DataTool.ConvertLogic {
    public static class Sound {
        private static bool ArraysEqual(IReadOnlyCollection<byte> a1, IReadOnlyList<byte> a2) {
            ...
            return !a1.Where((t, i) => !comparer.Equals(t, a2[i]))
                      .Any();
        }
        ...
        public static ushort SwapBytes(ushort x) { return (ushort) SwapBytes((uint) x); }
        ...
        public class Packet {
            ...
            public Packet(BinaryReader reader,
                ...
                if (littleEndian) {
                    ...
                    // _size = read_16_le(i);
                    if (!_noGranule) _absoluteGranule = reader.ReadUInt32();
                } else {
                    Debugger.Break();
                    _size = SwapBytes(reader.ReadUInt16());
                    if (!_noGranule) _absoluteGranule = SwapBytes(reader.ReadUInt32());
                }
            }

            public long HeaderSize() { return _noGranule ? 2 : 6; }

            public long Offset() { return _offset + HeaderSize(); }

            public ushort Size() { return _size; }

            public uint Granule() { return _absoluteGranule; }

            public long NextOffset() { return _offset + HeaderSize() + _size; }
        }

        public class VorbisPacketHeader {
            public readonly char[] VorbisStr = {
                                                   'v',
                                                   'o',
                                                   'r',
                                                   'b',
            ...
            public VorbisPacketHeader(byte type) { Type = type; }
        }
        ...
        public class WwiseRIFFVorbis : IDisposable {
            ...
            public WwiseRIFFVorbis(Stream stream, string codebooksFile) {
                ...
                if (new string(_riffHead) != "RIFX") {
                    if (new string(_riffHead) != "RIFF") throw new Exception("missing RIFF");

                    _littleEndian = true;
                }

                ushort Read16() { return _littleEndian ? _reader.ReadUInt16() : SwapBytes(_reader.ReadUInt16()); }

                uint Read32() { return _littleEndian ? _reader.ReadUInt32() : SwapBytes(_reader.ReadUInt32()); }
                ...
                while (_chunkOffset < _riffSize) {
                    ...
                    var chunkType       = _reader.ReadChars(4);
                    var chunkTypeString = new string(chunkType);
                    var chunkSize       = Read32();
                ...
                if (-1 == _vorbOffset && 0x42 == _fmtSize) _vorbOffset = _fmtOffset + 0x18;
                ...
                if (_fmtSize - 0x12 >= 2) {
                    // read extra fmt
                    _extUnk = Read16();
                    if (_fmtSize - 0x12 >= 6) _subtype = Read32();
                }

                if (_fmtSize == 0x28) {
                    byte[] whoknowsbufCheck = {
                                                  1,
                                                  0,
                                                  0,
                                                  0,
                    ...
                    var whoknowsbuf = _reader.ReadBytes(16);
                ...
                switch (_vorbSize) {
                    ...
                    case 0x2A:
                        ...
                        if (0x4A != modSignal && 0x4B != modSignal && 0x69 != modSignal && 0x70 != modSignal) _modPackets = true;
            ...
            private void GenerateOggHeader(BitOggstream os, out bool[] modeBlockflag, out int modeBits) {
                ...
                {
                    var vhead = new VorbisPacketHeader(1);
                    os.Write(vhead);

                    var version = new BitUint(32, 0);
                    os.Write(version);

                    var ch = new BitUint(8, _channels);
                    os.Write(ch);

                    var srate = new BitUint(32, _sampleRate);
                    os.Write(srate);

                    var bitrateMax = new BitUint(32, 0);
                    os.Write(bitrateMax);

                    var bitrateNominal = new BitUint(32, _avgBytesPerSecond * 8);
                    os.Write(bitrateNominal);

                    var bitrateMinimum = new BitUint(32, 0);
                    os.Write(bitrateMinimum);

                    var blocksize0 = new BitUint(4, _blocksize0Pow);
                    os.Write(blocksize0);

                    var blocksize1 = new BitUint(4, _blocksize1Pow);
                    os.Write(blocksize1);

                    var framing = new BitUint(1, 1);
                    os.Write(framing);
                ...
                // Comment packet
                {
                    ...
                    var vhead = new VorbisPacketHeader(3);
                    os.Write(vhead);
                    ...
                    var vendorSize = new BitUint(32, (uint) vendor.Length);
                    os.Write(vendorSize);

                    foreach (var vendorChar in vendor) {
                        var c = new BitUint(8, (byte) vendorChar);
                        os.Write(c);
                    }

                    if (_loopCount == 0) {
                        var userCommentCount = new BitUint(32, 0);
                        os.Write(userCommentCount);
                    } else {
                        var comments         = new List<string> { $"LoopStart={_loopStart}", $"LoopEnd={_loopEnd}" };
                        var userCommentCount = new BitUint(32, (uint) comments.Count);
                        os.Write(userCommentCount);

                        foreach (var comment in comments) {
                            var commentLength = new BitUint(32, (uint) comment.Length);
                            os.Write(commentLength);

                            foreach (var c in comment) {
                                var charBit = new BitUint(8, (byte) c);
                                os.Write(charBit);
                            }
                        }
                    }
                ...
                {
                    var vhead = new VorbisPacketHeader(5);
                    os.Write(vhead);

                    var setupPacket = new Packet(_reader, _dataOffset + _setupPacketOffset, _littleEndian, _noGranule);

                    _reader.BaseStream.Position = setupPacket.Offset();
                    if (setupPacket.Granule() != 0) throw new Exception("setup packet granule != 0");

                    var ss = new BitStream(_reader);

                    var codebookCountLess1 = new BitUint(8);
                    ...
                    var library = new CodebookLibrary(_codebooksFile);

                    for (var i = 0; i < codebookCount; i++) {
                        var codebookID = new BitUint(10);
                        ss.Read(codebookID);
                        try {
                            ...
                            var codebookIdentifier = new BitUint(14);
                            ...
                            //                   X
                            //         01 0101 10 01 0000
                            if (codebookIdentifier == 0x159) throw new Exception("invalid codebook id 0x342, try --full-setup");
                    ...
                    var timeCountLess1 = new BitUint(6, 0);
                    os.Write(timeCountLess1);
                    var dummyTimeValue = new BitUint(16, 0);
                    ...
                    {
                        // floor count
                        var floorCountLess1 = new BitUint(6);
                        ...
                        for (uint i = 0; i < floorCount; i++) {
                            var floorType = new BitUint(16, 1);
                            os.Write(floorType);

                            var floor1Partitions = new BitUint(5);
                            ...
                            for (var j = 0; j <= maximumClass; j++) {
                                ...
                                for (uint k = 0; k < 1U << classSubclasses.AsInt(); k++) {
                                    var subclassBookPlus1 = new BitUint(8);
                            ...
                            var floor1MultiplierLess1 = new BitUint(2);
                            ss.Read(floor1MultiplierLess1);
                            os.Write(floor1MultiplierLess1);

                            var rangebits = new BitUint(4);
                            ss.Read(rangebits);
                            os.Write(rangebits);
                        ...
                        // residue count
                        var residueCountLess1 = new BitUint(6);
                        ss.Read(residueCountLess1);
                        var residueCount = residueCountLess1 + 1;
                        os.Write(residueCountLess1);

                        for (uint i = 0; i < residueCount; i++) {
                            var residueType = new BitUint(2);
                            ss.Read(residueType);
                            os.Write(new BitUint(16, residueType));

                            if (residueType > 2) throw new Exception("invalid residue type");

                            var residueBegin                = new BitUint(24);
                            var residueEnd                  = new BitUint(24);
                            var residuePartitionSizeLess1   = new BitUint(24);
                            var residueClassificationsLess1 = new BitUint(6);
                            var residueClassbook            = new BitUint(8);
                            ...
                            for (uint j = 0; j < residueClassifications; j++) {
                                var highBits = new BitUint(5, 0);
                                var lowBits  = new BitUint(3);

                                ss.Read(lowBits);
                                os.Write(lowBits);

                                var bitFlag = new BitUint(1);
                                ss.Read(bitFlag);
                                os.Write(bitFlag);
                                if (bitFlag == 1) {
                                    ss.Read(highBits);
                        ...
                        var mappingCountLess1 = new BitUint(6);
                        ...
                        for (uint i = 0; i < mappingCount; i++) {
                            ...
                            if (squarePolarFlag == 1) {
                                ...
                                for (uint j = 0; j < couplingSteps; j++) {
                                    var magnitude = new BitUint((uint) Ilog((uint) (_channels - 1)));
                                    var angle     = new BitUint((uint) Ilog((uint) (_channels - 1)));
                            ...
                            var mappingReserved = new BitUint(2);
                            ...
                            for (uint j = 0; j < submaps; j++) {
                                var timeConfig = new BitUint(8);
                                ss.Read(timeConfig);
                                os.Write(timeConfig);

                                var floorNumber = new BitUint(8);
                                ss.Read(floorNumber);
                                os.Write(floorNumber);

                                if (floorNumber >= floorCount) throw new Exception("invalid floor mapping");

                                var residueNumber = new BitUint(8);
                        ...
                        // mode count
                        var modeCountLess1 = new BitUint(6);
                        ...
                        for (uint i = 0; i < modeCount; i++) {
                            ...
                            var windowType    = new BitUint(16, 0);
                            var transformType = new BitUint(16, 0);
                            os.Write(windowType);
                            os.Write(transformType);

                            var mapping = new BitUint(8);
                            ss.Read(mapping);
                            os.Write(mapping);
            ...
            public void ConvertToOgg(Stream outputStream) {
                using (var writer = new BinaryWriter(outputStream, Encoding.Default, true)) { // leave open
                    outputStream.SetLength(0);
                    var os = new BitOggstream(writer);
                    ...
                    // audio pages
                    {
                        ...
                        while (offset < _dataOffset + _dataSize) {
                            ...
                            if (_oldPacketHeaders) throw new Exception("unsupported (if this happens please message the devs)");

                            {
                                var audioPacket = new Packet(_reader, offset, _littleEndian, _noGranule);
                            ...
                            if (offset + packetHeaderSize > _dataOffset + _dataSize) throw new Exception("page header truncated");
                            ...
                            if (_modPackets) {
                                if (modeBlockflag == null) throw new Exception("didn't load blockflag");

                                var packetType = new BitUint(1, 0);
                                os.Write(packetType);
                                ...
                                if (modeBlockflag[modeNumberP]) {
                                    ...
                                    if (nextOffset + packetHeaderSize <= _dataOffset + _dataSize) {
                                        ...
                                        var  audioPacket    = new Packet(_reader, nextOffset, _littleEndian, _noGranule);
                                        ...
                                        if (nextPacketSize > 0) {
                                            ...
                                            var ss             = new BitStream(_reader);
                                            var nextModeNumber = new BitUint((uint) modeBits);
                                    ...
                                    var prevWindowType = new BitUint(1, (uint) (prevBlockflag ? 1 : 0));
                                    os.Write(prevWindowType);

                                    var nextWindowType = new BitUint(1, (uint) (nextBlockflag ? 1 : 0));
                                ...
                                if (v < 0) throw new Exception("file truncated");
                                var c = new BitUint(8, (uint) v);
                                os.Write(c);
                            }
                        ...
                        if (offset > _dataOffset + _dataSize) throw new Exception("page truncated");
                    }
                }
            }
        }
        ...
        public class BankObjectAttribute : Attribute {
            ...
            public BankObjectAttribute(byte type) { Type = type; }
        }
        ...
        public class BankObjectEventAction : IBankObject {
            ...
            public void Read(BinaryReader reader) {
                ...
                var tempTypes = new EventActionParameterType[parameterCount];
                for (var i = parameterCount - 1; i >= 0; i--) {
                    var parameterType = (EventActionParameterType) reader.ReadByte();
                    tempTypes[i] = parameterType;
                }

                foreach (var parameterType in tempTypes) {
                    object val;
        ...
        public class BankObjectSettings : IBankObject {
            ...
            public void Read(BinaryReader reader) {
                var numSettings = reader.ReadByte();

                var types = new SettingType[numSettings];
                for (var i = 0; i < numSettings; i++) {
                    var type = (SettingType) reader.ReadByte();
                    types[i] = type;
                }
        ...
        public class BankObjectEvent : IBankObject {
            ...
            public void Read(BinaryReader reader) {
                ...
                for (var i = 0; i < numActions; i++) Actions[i] = reader.ReadUInt32();
            }
        }
        ...
        public class WwiseBank {
            public WwiseBank(Stream stream) {
                ...
                using (var reader = new BinaryReader(stream, Encoding.Default, true)) {
                    ...
                    while (reader.BaseStream.Position < reader.BaseStream.Length) {
                        var chunk = reader.Read<WwiseBankChunkHeader>();
                        Chunks.Add(chunk);
                        ChunkPositions[chunk]      =  reader.BaseStream.Position;
                        reader.BaseStream.Position += chunk.ChunkLength;
                    }

                    var dataHeader = Chunks.FirstOrDefault(x => x.Name == "DATA");
                    var didxHeader = Chunks.FirstOrDefault(x => x.Name == "DIDX");
                    ...
                    if (dataHeader.MagicNumber != 0 && dataHeader.MagicNumber != 0) {
                        ...
                        for (var i = 0; i < didxHeader.ChunkLength / 12; i++) {
                            ...
                            WemData[i] = reader.ReadBytes(WemDefs[i]
                                                              .FileLength);
                    ...
                    var hircHeader = Chunks.FirstOrDefault(x => x.Name == "HIRC");
                    ...
                    if (hircHeader.MagicNumber != 0) {
                        ...
                        for (var o = 0; o < objectCount; o++) {
                            ...
                            var beforeObject = reader.BaseStream.Position;
                            ...
                            var newPos = beforeObject + objectLength;
            ...
            public static void GetReady() {
                ...
                var assembly = typeof(WwiseBank).Assembly;
                var baseType = typeof(IBankObject);
                var types = assembly.GetTypes()
                                    .Where(type => type != baseType && baseType.IsAssignableFrom(type))
                                    .ToList();

                foreach (var type in types) {
                    var bankObjectAttribute = type.GetCustomAttribute<BankObjectAttribute>();
            ...
            public void WriteWems(string output) {
                ...
                for (var i = 0; i < WemDefs.Length; i++) {
                    var wemDef = WemDefs[i];
                    var data   = WemData[i];
            ...
            public IEnumerable<T> ObjectsOfType<T>() {
                foreach (var bankObject in Objects)
    ...
    public class BitOggstream : IDisposable {
        ...
        private readonly byte[] _pageBuffer = new byte[(int) SizeEnum.HeaderBytes + (int) SizeEnum.MaxSegments + (int) SizeEnum.SegmentSize * (int) SizeEnum.MaxSegments];
        ...
        public void Dispose() { FlushPage(); }

        public void PutBit(bool bit) {
            if (bit) _bitBuffer |= (byte) (1 << (byte) _bitsStored);
        ...
        public void FlushBits() {
            ...
            if (_payloadBytes == (int) SizeEnum.SegmentSize * (int) SizeEnum.MaxSegments) throw new Exception("ran out of space in an Ogg packet");

            _pageBuffer[(int) SizeEnum.HeaderBytes + (int) SizeEnum.MaxSegments + _payloadBytes] = _bitBuffer;
            _payloadBytes++;
        ...
        private void Write32Le(IList<byte> bytes, int startIndex, uint val) {
            var valBytes = Int2Le(val);
            for (var i = 0; i < 4; i++) bytes[i + startIndex] = valBytes[i];
        ...
        public void FlushPage(bool nextContinued = false, bool last = false) {
            if (_payloadBytes != (int) SizeEnum.SegmentSize * (int) SizeEnum.MaxSegments) FlushBits();

            if (_payloadBytes != 0) {
                var segments                                             = (_payloadBytes + (int) SizeEnum.SegmentSize) / (int) SizeEnum.SegmentSize; // intentionally round up
                if (segments == (int) SizeEnum.MaxSegments + 1) segments = (int) SizeEnum.MaxSegments;                                                // at max eschews the final 0

                // move payload back
                for (uint i = 0; i < _payloadBytes; i++) _pageBuffer[(int) SizeEnum.HeaderBytes + segments + i] = _pageBuffer[(int) SizeEnum.HeaderBytes + (int) SizeEnum.MaxSegments + i];
                ...
                if (_granule == 0xFFFFFFFF) Write32Le(_pageBuffer, 10, 0xFFFFFFFF);
                ...
                // output to ostream
                for (uint i = 0; i < (int) SizeEnum.HeaderBytes + segments + _payloadBytes; i++) _os.Write(_pageBuffer[i]);
        ...
        public void Write(Sound.VorbisPacketHeader vph) {
            var t = new BitUint(8, vph.Type);
            Write(t);

            for (uint i = 0; i < 6; i++) {
                var c = new BitUint(8, (byte) vph.VorbisStr[i]);
                Write(c);
            }
        }
    ...
    public class BitUint {
        ...
        public static implicit operator uint(BitUint bitUint) { return bitUint.Value; }
    ...
    public class BitStream : IDisposable {
        ...
        public BitStream(BinaryReader reader) { _reader = reader; }
        ...
        public void Read(BitUint bitUint) {
            bitUint.Value = 0;
            for (var i = 0; i < bitUint.BitSize; i++)
                if (GetBit())
                    bitUint.Value |= 1U << i;
        }
    }
    ...
    public class CodebookLibrary {
        ...
        public CodebookLibrary(string file) {
            ...
            using (Stream codebookStream = System.IO.File.OpenRead(file)) {
                using (var reader = new BinaryReader(codebookStream)) {
                    var fileSize = codebookStream.Length;
                    ...
                    for (var i = 0; i < offsetOffset; i++) CodebookData[i] = reader.ReadByte();

                    for (var i = 0; i < _codebookCount; i++) CodebookOffsets[i] = reader.ReadInt32();
                }
            }
        }

        public void Rebuild(int codebookID, BitOggstream os) {
            var   cbIndexStart = GetCodebook(codebookID);
            ulong cbSize;

            {
                var signedCbSize = GetCodebookSize(codebookID);
                if (cbIndexStart == null || -1 == signedCbSize) throw new InvalidID();
                cbSize = (ulong) signedCbSize;
            }

            var cbStartIndex = (long) cbIndexStart;
            var unsignedSize = (long) cbSize;

            Stream codebookStream = new MemoryStream();
            for (var i = cbStartIndex; i < unsignedSize + cbStartIndex; i++) codebookStream.WriteByte(CodebookData[i]);

            var reader    = new BinaryReader(codebookStream);
            var bitStream = new BitStream(reader);
        ...
        public void Rebuild(BitStream bis, ulong cbSize, BitOggstream bos) {
            ...
            var dimensions = new BitUint(4);
            var entries    = new BitUint(14);
            bis.Read(dimensions);
            bis.Read(entries);
            ...
            var ordered = new BitUint(1);
            bis.Read(ordered);
            bos.Write(ordered);

            if (ordered == 1) {
                /* IN/OUT: 5 bit initial length */
                var initialLength = new BitUint(5);
                ...
                while (currentEntry < entries) {
                    ...
                    var number = new BitUint((uint) Sound.WwiseRIFFVorbis.Ilog((uint) (entries - currentEntry)));
                    bis.Read(number);
                    bos.Write(number);
                ...
                var codewordLengthLength = new BitUint(3);
                var sparse               = new BitUint(1);
                bis.Read(codewordLengthLength);
                bis.Read(sparse);

                if (0 == codewordLengthLength || 5 < codewordLengthLength) throw new Exception("nonsense codeword length");
                ...
                for (var i = 0; i < entries; i++) {
                    ...
                    if (presentBool) {
                        ...
                        var codewordLength = new BitUint(codewordLengthLength);
            ...
            if (lookupType == 0) {
                ...
                var min          = new BitUint(32);
                var max          = new BitUint(32);
                var valueLength  = new BitUint(4);
                var sequenceFlag = new BitUint(1);
                bis.Read(min);
                bis.Read(max);
                ...
                var quantvals = _bookMaptype1Quantvals(entries, dimensions);
                for (uint i = 0; i < quantvals; i++) {
                    /* IN/OUT: n bit value */
                    var val = new BitUint(valueLength + 1);
                    bis.Read(val);
                    bos.Write(val);
                }
            }
            ...
            if (0 != cbSize && bis.TotalBitsRead / 8 + 1 != (int) cbSize) throw new Exception($"{cbSize}, {bis.TotalBitsRead / 8 + 1}");
        }

        private uint _bookMaptype1Quantvals(uint entries, uint dimensions) {
            /* get us a starting hint, we'll polish it below */
            var bits = Sound.WwiseRIFFVorbis.Ilog(entries);
