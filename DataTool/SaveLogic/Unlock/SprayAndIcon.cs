...
namespace DataTool.SaveLogic.Unlock {
    public static class SprayAndIcon {
        public static void Save(ICLIFlags flags, string directory, DataModels.Unlock unlock) {
            ...
            try {
                info.Textures.First(x => x.Value.Loose)
                    .Value.Name = unlock.Name;
            } catch {
                // what
            }

            var extractFlags = flags as ExtractFlags;

            if (extractFlags?.SprayOnlyImage == true) directory = Path.GetFullPath(Path.Combine(directory, ".."));
