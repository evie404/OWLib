...
namespace DataTool.SaveLogic.Unlock {
    public static class SkinTheme {
        ...
        public static void Save(ICLIFlags    flags,
                                ...
                                STU_63172E83 skin,
                                STUHero      hero) {
            var skinTheme = GetInstance<STUSkinTheme>(skin.m_5E9665E3);
            if (skinTheme == null) return;
        ...
        public static void Save(ICLIFlags   flags,
                                ...
                                STUSkinBase skin,
                                STUHero     hero) {
            var replacements = GetReplacements(skin);
            ...
            if (skin is STUSkinTheme skinTheme) {
                ...
                foreach (var weaponOverrideGUID in skinTheme.m_heroWeapons) {
                    var heroWeapon = GetInstance<STUHeroWeapon>(weaponOverrideGUID);
                    if (heroWeapon == null) continue;

                    var weaponReplacements = GetReplacements(heroWeapon);
            ...
            if (replacements != null) {
                var soundDirectory = Path.Combine(directory, "Sound");

                var diffInfoBefore = new FindLogic.Combo.ComboInfo();
                var diffInfoAfter  = new FindLogic.Combo.ComboInfo();

                foreach (var replacement in replacements) {
                    uint diffReplacementType = teResourceGUID.Type(replacement.Value);
                    if (diffReplacementType != 0x2C && diffReplacementType != 0x3F && diffReplacementType != 0xB2) continue; // no voice sets, use extract-hero-voice
                    FindLogic.Combo.Find(diffInfoAfter,  replacement.Value);
                    FindLogic.Combo.Find(diffInfoBefore, replacement.Key);
                }

                foreach (var voiceSet in diffInfoAfter.VoiceSets) {
                ...
                foreach (var soundFile in diffInfoAfter.SoundFiles) {
                ...
                foreach (var soundFile in diffInfoAfter.VoiceSoundFiles) {
        ...
        private static void SavePreviewWeapons(FindLogic.Combo.ComboInfo info, Dictionary<ulong, ulong> weaponReplacements, STU_A0872511[] entities) {
            ...
            foreach (var weaponEntity in entities) {
                ...
                var loadout = new Loadout(weaponEntity.m_loadout);
        ...
        public static Dictionary<ulong, ulong> GetReplacements(STUSkinBase skin) {
            if (skin.m_runtimeOverrides != null) {
                ...
                foreach (var @override in skin.m_runtimeOverrides) replacements[@override.Key] = @override.Value.m_3D884507;
                return replacements;
            }
