...
namespace DataTool.SaveLogic.Unlock {
    public static class PortraitFrame {
        public static void Save(ICLIFlags flags, string directory, DataModels.Unlock unlock) {
            var portraitFrame = (STUUnlock_PortraitFrame) unlock.STU;

            var tier = portraitFrame.m_rank.ToString();
            ...
            if (portraitFrame.m_949D9C2A != null) {
                ...
                var borderNum = portraitFrame.m_level - portraitFrame.m_stars * 10 - (int) portraitFrame.m_rank * 10;

                if ((int) portraitFrame.m_rank > 1) borderNum -= 50 * ((int) portraitFrame.m_rank - 1);
                borderNum -= 1;
