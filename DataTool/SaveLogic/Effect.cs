...
namespace DataTool.SaveLogic {
    public static class Effect {
        public class OverwatchEffect : IExportFormat {
            ...
            public OverwatchEffect(FindLogic.Combo.ComboInfo info, FindLogic.Combo.EffectInfoCombo effectInfo, Dictionary<ulong, HashSet<FindLogic.Combo.VoiceLineInstanceInfo>> voiceStimuli) {
                Info         = info;
            ...
            protected void WriteEffect(BinaryWriter writer) {
                ...
                foreach (var dmceInfo in effect.DMCEs) {
                    ...
                    var modelInfo = Info.Models[dmceInfo.Model];
                    writer.Write($"Models\\{modelInfo.GetName()}\\{modelInfo.GetNameIndex()}.owmdl");
                    if (dmceInfo.Animation == 0) {
                        writer.Write("null");
                    } else {
                        var animationInfo = Info.Animations[dmceInfo.Animation];
                ...
                foreach (var ceceInfo in effect.CECEs) {
                    ...
                    if (ceceInfo.Animation != 0) {
                        var animationInfo = Info.Animations[ceceInfo.Animation];
                ...
                foreach (var svceInfo in effect.SVCEs) {
                    ...
                    if (VoiceStimuli.ContainsKey(svceInfo.VoiceStimulus)) {
                        var lines = VoiceStimuli[svceInfo.VoiceStimulus];
                        writer.Write(lines.Count);

                        foreach (var voiceLineInstance in lines) {
                            writer.Write(voiceLineInstance.SoundFiles.Count);
                            foreach (var soundFile in voiceLineInstance.SoundFiles) {
                                var soundFileInfo = Info.VoiceSoundFiles[soundFile];
