...
namespace DataTool.ToolLogic.Extract {
    ...
    public class ExtractGeneral : ITool {
        ...
        public void GetGeneralUnlocks(ICLIFlags toolFlags) {
            ...
            var path = Path.Combine(basePath, "General");

            foreach (var key in TrackedFiles[0x54]) {
                var progression = GetInstance<STUGenericSettings_PlayerProgression>(key);
                if (progression == null) continue;

                var playerProgression = new PlayerProgression(progression);
                ...
                if (playerProgression.OtherUnlocks != null) ExtractHeroUnlocks.SaveUnlocks(flags, playerProgression.OtherUnlocks, path, "Achievement", null, null, null, null);
            }
        }
    }
}
