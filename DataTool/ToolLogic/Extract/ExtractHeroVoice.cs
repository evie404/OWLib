...
namespace DataTool.ToolLogic.Extract {
    ...
    public class ExtractHeroVoice : QueryParser, ITool, IQueryParser {
        ...
        public void Parse(ICLIFlags toolFlags) { SaveHeroSounds(toolFlags); }
        ...
        private void SaveHeroSounds(ICLIFlags toolFlags) {
            ...
            foreach (var heroFile in TrackedFiles[0x75]) {
                var hero = GetInstance<STUHero>(heroFile);
                if (hero == null) continue;

                var heroNameActual = (GetString(hero.m_0EDCE350) ?? $"Unknown{teResourceGUID.Index(heroFile)}").TrimEnd(' ');
                ...
                if (SaveSet(flags, basePath, hero.m_gameplayEntity, heroFileName, "Default", ref baseComponent, ref baseInfo)) {
                    ...
                    var npc = progression.LootBoxesUnlocks == null;

                    foreach (var itemInfo in progression.OtherUnlocks) ProcessUnlock(itemInfo, flags, basePath, heroFileName, hero, baseComponent, baseInfo);
                    ...
                    foreach (var unlock in defaultUnlocks.Unlocks)
                    ...
                    foreach (var eventUnlocks in progression.LootBoxesUnlocks) {
                        ...
                        foreach (var unlock in eventUnlocks.Unlocks) ProcessUnlock(unlock, flags, basePath, heroFileName, hero, baseComponent, baseInfo);
                    }
                }
            }
        }

        private static void ProcessUnlock(Unlock               unlock,
                                          ICLIFlags            flags,
                                          string               basePath,
                                          string               heroFileName,
                                          STUHero              hero,
                                          STUVoiceSetComponent baseComponent,
                                          Combo.ComboInfo      baseInfo) {
        ...
        private static void SaveSkin(ICLIFlags            flags,
                                     ulong                skinResource,
                                     string               basePath,
                                     STUHero              hero,
                                     string               heroFileName,
                                     ...
                                     STUVoiceSetComponent baseComponent,
                                     Combo.ComboInfo      baseInfo) {
            var skin = GetInstance<STUSkinTheme>(skinResource);
            if (skin == null)
                return;
            ...
            SaveSet(flags, basePath, hero.m_gameplayEntity, heroFileName, GetValidFilename(name), ref component, ref info, baseComponent, baseInfo, SkinTheme.GetReplacements(skin));
        }

        private static bool SaveSet(ICLIFlags                flags,
                                    string                   basePath,
                                    ulong                    entityMain,
                                    string                   heroFileName,
                                    string                   skin,
                                    ref STUVoiceSetComponent voiceSetComponent,
                                    ref Combo.ComboInfo      info,
                                    STUVoiceSetComponent     baseComponent = null,
                                    Combo.ComboInfo          baseCombo     = null,
