...
using DataTool.WPF;
using DataTool.WPF.Tool.Export;
using TankLib;
...
namespace DataTool.ToolLogic.Extract {
    ...
    public class ExtractHeroUnlocks : QueryParser, IAwareTool, IQueryParser {
        ...
        public Task<Control> GetToolControl(ProgressWorker worker, SynchronizationContext context, Window window) { return HeroUnlocksView.Get(worker, context, window, NPCs); }
        ...
        public static IReadOnlyDictionary<teResourceGUID, string> GetEventConfig() {
            if (EventConfig != null) return EventConfig;

            var stu = OpenSTUSafe(TrackedFiles[0x54]
                                      .First(x => teResourceGUID.Index(x) == 0x16C));
            var map = stu.GetInstance<STU_D7BD8322>();
            EventConfig = map.m_categories.ToDictionary(x => x.m_id.GUID, y => GetString(y.m_name));
            return EventConfig;
        }

        public void SaveUnlocksForHeroes(ICLIFlags            flags,
                                         IEnumerable<STUHero> heroes,
                                         string               basePath,
                                         bool                 npc = false) {
            ...
            foreach (var hero in heroes) {
                ...
                var heroFileName = GetValidFilename(heroNameActual);

                if (config.Count == 0) continue;

                var heroPath = Path.Combine(basePath, RootDir, heroFileName);

                var voiceSet           = new VoiceSet(hero);
                ...
                {
                    var guiInfo = new Combo.ComboInfo();

                    foreach (var tex in hero.m_8203BFE1) {
                ...
                if (progressionUnlocks.OtherUnlocks != null) { // achievements and stuff
                    var tags = new Dictionary<string, TagExpectedValue> { { "event", new TagExpectedValue("base") } };
                ...
                if (npc) {
                    foreach (var skin in hero.m_skinThemes) {
                        if (!config.ContainsKey("skin") &&
                            config["skin"]
                                .ShouldDo(GetFileName(skin.m_5E9665E3)))
                            continue;
                        SkinTheme.Save(flags, Path.Combine(heroPath, Unlock.GetTypeName(typeof(STUUnlock_SkinTheme)), string.Empty, GetFileName(skin.m_5E9665E3)), skin, hero);
                    }
                ...
                if (progressionUnlocks.LevelUnlocks != null) { // default unlocks
                    var tags = new Dictionary<string, TagExpectedValue> { { "event", new TagExpectedValue("base") } };
                    foreach (var levelUnlocks in progressionUnlocks.LevelUnlocks) SaveUnlocks(flags, levelUnlocks.Unlocks, heroPath, "Default", config, tags, voiceSet, hero);
                }

                if (progressionUnlocks.LootBoxesUnlocks != null)
                    foreach (var lootBoxUnlocks in progressionUnlocks.LootBoxesUnlocks) {
                        if (lootBoxUnlocks.Unlocks == null) continue;
                        var lootboxName = LootBox.GetName(lootBoxUnlocks.LootBoxType);

                        var tags = new Dictionary<string, TagExpectedValue> { { "event", new TagExpectedValue(LootBox.GetBasicName(lootBoxUnlocks.LootBoxType)) } };
        ...
        public static void SaveUnlocks(ICLIFlags                            flags,
                                       Unlock[]                             unlocks,
                                       string                               path,
                                       string                               eventKey,
                                       Dictionary<string, ParsedArg>        config,
                                       Dictionary<string, TagExpectedValue> tags,
                                       VoiceSet                             voiceSet,
                                       STUHero                              hero) {
            if (unlocks == null) return;
            foreach (var unlock in unlocks) SaveUnlock(flags, unlock, path, eventKey, config, tags, voiceSet, hero);
        }

        public static void SaveUnlock(ICLIFlags                            flags,
                                      Unlock                               unlock,
                                      string                               path,
                                      string                               eventKey,
                                      Dictionary<string, ParsedArg>        config,
            ...
            if (tags != null) {
                if (unlock.STU.m_0B1BA7C1 == null) {
                    ...
                    var teamDef = new TeamDefinition(unlock.STU.m_0B1BA7C1);
            ...
            if (unlock.STU.m_BEE9BCDA != null) {
                ...
                if (eventMap.ContainsKey(formalEventKey)) eventKey = eventMap[formalEventKey];
            }

            var thisPath = Path.Combine(path, unlock.GetTypeNameEnum(), eventKey, rarity, GetValidFilename(unlock.GetName()));

            if (ShouldDo(unlock, config, tags, typeof(STUUnlock_SprayPaint))) SprayAndIcon.Save(flags,     thisPath, unlock);
            if (ShouldDo(unlock, config, tags, typeof(STUUnlock_AvatarPortrait))) SprayAndIcon.Save(flags, thisPath, unlock);

            if (ShouldDo(unlock, config, tags, typeof(STUUnlock_POTGAnimation))) AnimationItem.Save(flags, thisPath, unlock);
            if (ShouldDo(unlock, config, tags, typeof(STUUnlock_Emote))) AnimationItem.Save(flags,         thisPath, unlock);
            if (ShouldDo(unlock, config, tags, typeof(STUUnlock_Pose))) AnimationItem.Save(flags,          thisPath, unlock);

            if (ShouldDo(unlock, config, tags, typeof(STUUnlock_VoiceLine))) VoiceLine.Save(flags, thisPath, unlock, voiceSet);

            if (ShouldDo(unlock, config, tags, typeof(STUUnlock_SkinTheme))) SkinTheme.Save(flags, thisPath, unlock, hero);
        ...
        private static bool ShouldDo(Unlock                               unlock,
                                     Dictionary<string, ParsedArg>        config,
                                     Dictionary<string, TagExpectedValue> tags,
                                     Type                                 unlockType) {
            var type      = Unlock.GetTypeName(unlockType);
            var typeLower = type.ToLowerInvariant();
            if (config == null) return unlock.Type == type;
            return unlock.Type == type           &&
                   config.ContainsKey(typeLower) &&
                   config[typeLower]
                       .ShouldDo(unlock.GetName(), tags);
        }
    }
}
