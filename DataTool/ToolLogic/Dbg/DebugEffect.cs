...
namespace DataTool.ToolLogic.Dbg {
    ...
    internal class DebugEffect : ITool {
        ...
        private void SaveUnlock(ulong guid) {
            ...
            var potgAnim = unlock.STU as STUUnlock_POTGAnimation;
        ...
        private void SaveAnimation(string dir, ulong guid) {
            using (var animStream = OpenFile(guid)) {
                var animation = new teAnimation(animStream);
        ...
        private void SaveEffect(string dir, ulong guid) {
            using (var stream = OpenFile(guid)) {
                var chunkedData = new teChunkedData(stream);

                ulong lastModel = 0;

                foreach (var chunk in chunkedData.Chunks) {
                    if (chunk is teEffectChunkShaderSetup shaderSetup) ExtractDebugShaders.SaveMaterial(dir, shaderSetup.Header.Material, GetFileName(guid));
                ...
                foreach (var entityControl in chunkedData.GetChunks<teEffectComponentEntityControl>()) {
                    if (entityControl.Header.Animation == 0) continue;

                    SaveAnimation(dir, entityControl.Header.Animation);
                }

                foreach (var model in chunkedData.GetChunks<teEffectComponentModel>()) {
