...
namespace DataTool.ToolLogic.List.Debug {
    ...
    public class ListDebugHeroSettings : ITool {
        ...
        public void GetSoundbanks() {
            foreach (var key in Program.TrackedFiles[0x54]) {
                var settings = GetInstance<STUGenericSettings_HeroSettings>(key);
                if (settings == null) continue;

                var categories = new Dictionary<ulong, string>();

                foreach (var category in settings.m_categories) categories[category.m_identifier] = GetString(category.m_name);

                foreach (var heroSpecificSettings in settings.m_heroSpecificSettings) {
                    var hero = GetInstance<STUHero>(heroSpecificSettings.m_hero);
                    Console.Out.WriteLine($"{GetString(hero.m_0EDCE350)}:");

                    PrintSettings(settings.m_142A3CA9.Concat(heroSpecificSettings.m_settings)
                                          .ToArray(),
                                  categories);
                }
            }
        ...
        public void PrintSettings(STUHeroSettingBase[] settings, Dictionary<ulong, string> categories) {
            var settingCategories = new Dictionary<ulong, List<STUHeroSettingBase>>();
            foreach (var setting in settings) {
                if (!settingCategories.ContainsKey(setting.m_category)) settingCategories[setting.m_category] = new List<STUHeroSettingBase>();
                settingCategories[setting.m_category]
                    .Add(setting);
            }
                ...
                foreach (var setting in settingCategory.Value) {
                    ...
                    if (setting is STUHeroSettingIdentifier settingIdentifier)
                        foreach (var identifierEntry in settingIdentifier.m_entries)
                            ...
                            else if (identifierEntry is STU_714EA6C9 identifierEntryUx) Console.Out.WriteLine($"            {identifierEntryUx.m_name}");
                }
            }
        }
    }
